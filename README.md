...
- 打包完成后若浏览器没有弹出下载，面板会出现「**重新下载 zip**」按钮，点它重新下载（或检查浏览器地址栏的下载拦截提示）。
...
- **下载加固**：打包完成后的下载走 `GM_download`（扩展级下载 API，绕过 Chrome 对异步触发下载的拦截），不可用时回退页面 `a[download]`；下载没出现可点「重新下载」。看 `downloadBlob()`。
